...
public:
  bool filter_enable;
  float z_min;
  float z_max; // maximum height for 3D points
  ...
  float voxel_ly;
  float voxel_lz;
  // Clustering: keeping only clusters in which points are at least
  // cluster_tolerance [m] to each others
  bool cluster_enable;
  float cluster_tolerance;
  unsigned int cluster_min_size;
  unsigned int cluster_max_size;
  //gaussian mapping on z for intensity mapping
  bool gaussian_enable;
  float gaussian_mean; //usually set to robot height/middle of robot height
  float gaussian_stddev; //usually set to less or equal than half robot height
  // Normal mean averaging of maps
  bool normal_averaging_enable;
  // Moving average parameters
  bool moving_average_enable;
  float ma_alpha;
};
  ...
  void paramRefresh() {
    ...
    if (cell_num_x > 0) {
      RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"),
                         "Cells: " << cell_num_x << "*" << cell_num_y << "px ["
                                   << topleft_x << ", " << topleft_y << "]"
                                   << " [" << bottomright_x << ", "
                                   << bottomright_y << "]");
    }
