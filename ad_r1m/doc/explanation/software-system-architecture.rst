...
These features are combined into debian packages by the packaging logic defined in ``system/debian/``. For example, the ``ad-r1m-system-rpi5`` package, which installs all system-level files for the main AD-R1M variant, is a combination of the ``common``, ``rpi5`` and ``adrd3161`` system features. These compositions are defined in ``system/debian/<package>.install``, e.g.:

.. code-block::

    # system/debian/ad-r1m-system-rpi5.install
